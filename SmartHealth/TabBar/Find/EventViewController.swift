...
import UIKit
import Alamofire
class EventViewController: CommanViewController, UITableViewDelegate, UITableViewDataSource {
    var type: Int = Constants.Essay_List_Type_32
    @IBOutlet weak var tableView: UITableView!
    private var listModel: SHEssayInfoModel?
    private var listNotesModel: [SHEssayListInfoModel]  = []
    ...
    func loadData(){
        let parameters: Parameters = [
            ...
            Constants.List_OrderColumnName: Constants.Essay_List_OrderColumnName_Value,
            Constants.Essay_List_Type:type
            ]
        let request = Alamofire.request(Constants.EssayRetrieve,method: .get, parameters: parameters, encoding: URLEncoding.default,headers: ApiHelper.getDefaultHeader())
        ...
        request.responseJSON { response in
            ...
            case .success(let data):
                ...
                self.listModel = SHEssayInfoModel(JSON: dic as! [String : Any])
...
//        performSegue(withIdentifier: "toEventDetail", sender: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! FindEventListCell
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "toEventDetail") {
            ...
            if let nextViewController = segue.destination as? FindDetailViewController{
                nextViewController.noteModel = self.listNotesModel[cell.tag]
            }
        }
    }
